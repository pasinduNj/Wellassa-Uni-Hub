...
if (!empty($auth_key_cookie)) {
  ...
  $stmt = $conn->prepare("SELECT user_id, user_type, auth_key_expires FROM user WHERE auth_key = ?");
  $stmt->bind_param('s', $auth_key_cookie);
  $stmt->execute();
  $stmt->store_result();
  if ($stmt->num_rows > 0) {
    $stmt->bind_result($user_id, $user_type, $auth_key_expires);
    $stmt->fetch();
    if (strtotime($auth_key_expires) > time()) {
  ...
  <nav class="navbar navbar-expand-md sticky-top text-nowrap shadow visible navbar-shrink py-3 navbar-light" id="mainNav" style="
    ...
    <div class="container">
      <span class="bs-icon-sm bs-icon-circle bs-icon-primary shadow d-flex justify-content-center align-items-center me-2 bs-icon"><svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" fill="currentColor" viewBox="0 0 16 16" class="bi bi-gear-wide">
        ...
        </svg></span><a class="navbar-brand d-flex align-items-center" href="/"><span>Wellassa UniHUB</span></a><button data-bs-toggle="collapse" class="navbar-toggler" data-bs-target="#navcol-1">
      ...
      <div class="collapse navbar-collapse" id="navcol-1">
        <ul class="navbar-nav mx-auto">
          ...
          <li class="nav-item dropdown" style="margin: 0px; padding: -1px">
            <a class="dropdown-toggle nav-link d-lg-flex align-items-lg-center" aria-expanded="false" data-bs-toggle="dropdown" href="Reservations.html" target="_blank" style="
                  margin: -1px;
                  padding: 9px;
                  font-weight: bold;
            ...
            <div class="dropdown-menu">
              <a class="dropdown-item" href="Reservations.html">Reservations</a><a class="dropdown-item" href="product.html">Order Products</a><a class="dropdown-item" href="Freelance.html">Freelance</a>
            </div>
          </li>
          <li class="nav-item">
          ...
          <li class="nav-item">
            <a class="nav-link" href="Service%20Provider.html" style="margin: 0px; padding: 8px; height: 36.6px">Profile</a>
          </li>
        </ul>
        <a class="btn btn-primary shadow" role="button" href="signup.html" style="
              height: 35px;
              padding: 3px 10px;
              margin: 10px -10px 10px 10px;
              width: 78px;
              border-radius: 59px;
            ">Sign Up</a><a class="font-monospace link-warning border-white d-inline-block float-end d-lg-flex align-items-lg-center pull-right" role="button" href="shopping-cart.html" style="
              padding: 1px;
              padding-left: 11px;
              margin: 0px;
              margin-left: 7px;
